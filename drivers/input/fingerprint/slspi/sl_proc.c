...
static void sl_proc_cmd_aheap_stat_start(char *cmd)
{
	printk(KERN_ERR"thomas %s\n", __func__);
	sl_proc_cmd_uevent(cmd);
}
...
static void sl_proc_cmd_aheap_stat_stop(char *cmd)
{
	printk(KERN_DEBUG "%s\n", __func__);
	sl_proc_cmd_uevent(cmd);
}

static void sl_proc_cmd_art_irq_send(char *cmd)
{
	printk(KERN_DEBUG "%s\n", __func__);
	sl_proc_cmd_uevent(cmd);
}
...
static void sl_proc_cmd_uevent(char *cmd)
{
	char *event1[] = { "SILEAD_FP_EVENT=HEAP_START", NULL };
	char *event2[] = { "SILEAD_FP_EVENT=HEAP_STOP", NULL };
	char *event3[] = { "SILEAD_FP_EVENT=IRQ", NULL };
	if (fp_spidev) {
		if(!strncmp(cmd,"aheap_stat_start",strlen("aheap_stat_start"))){
			printk(KERN_ERR "thomas aheap_stat_start \n");
			kobject_uevent_env(&fp_spidev->spi->dev.kobj, KOBJ_CHANGE,event1);
		}else if(!strncmp(cmd,"aheap_stat_stop",strlen("aheap_stat_stop"))){
			printk(KERN_ERR "thomas aheap_stat_stop \n");
			kobject_uevent_env(&fp_spidev->spi->dev.kobj, KOBJ_CHANGE,event2);
		}else if(!strncmp(cmd,"art_irq_send",strlen("art_irq_send"))){
			printk(KERN_ERR "thomas art_irq_send\n");
			kobject_uevent_env(&fp_spidev->spi->dev.kobj, KOBJ_CHANGE,event3);
		}
	}
}
...
struct sl_proc_cmd sl_cmd[] = {
	...
	{"aheap_stat_start", sl_proc_cmd_aheap_stat_start},
	{"stop", sl_proc_cmd_stop},
	{"aheap_stat_stop", sl_proc_cmd_aheap_stat_stop},
	...
	{"art_irq_send", sl_proc_cmd_art_irq_send},
};
...
static void sl_proc_cmd_reg(char *cmd)
{
	...
	if (addr & (~0xFF)) {
		spidev_write_reg(fp_spidev, (addr >> 7), 0xF0);
		sprintf(fp_spidev->mmap_buf, "0x%08x=0x%08x", addr,
			spidev_read_reg(fp_spidev, addr % 0x80));
	} else {
		sprintf(fp_spidev->mmap_buf, "0x%08x=0x%08x", addr,	/*0xFF& */
	      ...
	      loff_t * f_pos)
{
	...
	printk(KERN_ERR "thomas [%s] entered!\n", __func__);
	if (cmd == NULL) {
		return 0;
	}
	memset(cmd, 0, count);
...
int sl_proc_init(struct spidev_data *spidev)
{
	...
	printk(KERN_ERR "[%s] proc file create pass !\n", __func__);
	if (NULL == sl_proc) {
