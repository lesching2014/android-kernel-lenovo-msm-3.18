...
static char fw_version_hlt[40] = {"ctp name:CTPFW_GT917D_HLT_0f.4016.4a"};
static char fw_version_ofg[40] = {"ctp name:CTPFW_GT917D_OFG_05.4016.4a"};
...
{
    ssize_t len = 0;
	if(gtp_module_id == 0x05)
		len +=sprintf(buff, "%s\n",fw_version_ofg);
	else if(gtp_module_id == 0x0f)
    	len +=sprintf(buff, "%s\n",fw_version_hlt);
    return len;
}
