...
#include <linux/irq.h>
#include "gt9xx.h"
//Huaqin add for Reduce the bright screen time by qimaokang at 2018/5/2 start
#include <linux/kthread.h>
//Huaqin add for Reduce the bright screen time by qimaokang at 2018/5/2 end
...
#endif
//huaqin modify by xudongfang for ito test at 2018/3/27 end
...
#endif
//huaqin modify by xudongfang for ito test at 2018/3/27 start
...
#endif
#if GTP_GESTURE_WAKEUP
static int gt9xx_gesture_enable = 0;
void gt9xx_gesture_en(int enable)
{
	gt9xx_gesture_enable = enable;
}
#else
void gt9xx_gesture_en(int enable)
{
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data *ts)
{
	...
	#else
	//Huaqin add for reset wake by zhangxiude at 2018/07/11 start
		gtp_irq_disable(ts);
		GTP_GPIO_OUTPUT(gtp_int_gpio, 1);
		gtp_reset_guitar(ts->client, 10);
		gtp_irq_enable(ts);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct goodix_ts_data *ts;
	u16 version_info;
//huaqin add for show firmware version by xudongfang at 2018/5/22 start
	u8 dfdev_info_fw[32]={'0'};
	u8 dfdev_info_cfg[32]={'0'};
	char temp_data[GTP_CONFIG_MAX_LENGTH + 2] = {0x80, 0x47};
//huaqin add for show firmware version by xudongfang at 2018/5/22 end
//huaqin add by xudongfang for disable vdd at 2018/3/5 start
	int df_ret;
//huaqin add by xudongfang for disable vdd at 2018/3/5 end
	GTP_DEBUG_FUNC();
...
#endif
//huaqin add for show firmware version by xudongfang at 2018/5/22 start
//sys/bus/platform/devices/hq_dev_info/    cat name
#ifdef CONFIG_HQ_DEV_INFO
	i2c_read_bytes(client, GTP_REG_VERSION, temp_data, 6);
	sprintf(dfdev_info_fw,"Chip PID:%c%c%c%c  VID:0x%02x%02x",
	temp_data[0],temp_data[1],temp_data[2],temp_data[3],temp_data[5],temp_data[4]);
	dev_info_register("CTP",dfdev_info_fw);
	i2c_read_bytes(client, GTP_REG_CONFIG_DATA, temp_data, 1);
	sprintf(dfdev_info_cfg,"ctp_config_version:%d,%#x",temp_data[0],temp_data[1]);
	dev_info_register("CTP",dfdev_info_cfg);
#endif
//huaqin add for show firmware version by xudongfang at 2018/5/22 end

	printk("GTP_DF_return\n");
//huaqin modify by xudongfang for ito test at 2018/3/27 end
...
#if GTP_GESTURE_WAKEUP
//huaqin add for gesture function by xudongfang at 2018/5/22 start
	if(gt9xx_gesture_enable == 1){
...
//huaqin add for gesture function by xudongfang at 2018/5/22 end
#else
	if (ts->use_irq) {
		gtp_irq_disable(ts);
	} else {
...
{
	s8 ret = -1;
	//Huaqin add for Plam sleep in/wake up by zhangxiude at 2018/07/06 start
        u32 finger_count = 0;
	//Huaqin add for Plam sleep in/wake up by zhangxiude at 2018/5/2 start
	GTP_DEBUG_FUNC();
	if (ts->enter_update) {
		return;
	}
	...
	input_sync(ts->input_dev);
	//Huaqin add for Plam sleep in/wake up  by zhangxiude at 2018/07/06 end
...
//Huaqin add for Reduce the bright screen time by qimaokang at 2018/5/2 start
int fb_gtx_ts_resume(void *data)
...
//Huaqin add for Reduce the bright screen time by qimaokang at 2018/5/2 start
			kthread_run(fb_gtx_ts_resume,ts,"tp_resume");
//Huaqin add for Reduce the bright screen time by qimaokang at 2018/5/2 end
