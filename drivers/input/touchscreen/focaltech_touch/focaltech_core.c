...
//huaqin del by xudongfang at 2018/3/7 start
...
//huaqin del by xudongfang at 2018/3/7 end
...
//Huaqin add for Reduce the bright screen time by qimaokang at 2018/5/2 start
int fb_fts_ts_resume(void *data)
{
...
//Huaqin add for Reduce the bright screen time by qimaokang at 2018/5/2 start
	    kthread_run(fb_fts_ts_resume,&fts_data->client->dev,"tp_resume");
//Huaqin add for Reduce the bright screen time by qimaokang at 2018/5/2 end
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    struct fts_ts_platform_data *pdata;
    struct fts_ts_data *ts_data;
//huaqin add by xudongfang for dev_info at 2018/3/15  start
	u8 dfval = 0;
	char dfdev_info[512]={'0'};
//huaqin add by xudongfang for dev_info at 2018/3/15  end
    FTS_FUNC_ENTER();
...
//huaqin add by xudongfang for temporary ito test at 2018/2/11 start
    ...
    ret = df_fts_test_init(&hwinfo_device);
    if (ret) {
        FTS_ERROR("DF_init production test fail");
    }
#endif
//huaqin add by xudongfang for temporary ito test at 2018/2/11 end
...
//huaqin add by xudongfang for dev_info at 2018/3/15 start
	...
	fts_i2c_read_reg(client, FTS_REG_FW_VER, &dfval);
    sprintf(dfdev_info, "   FW Ver:   %#x\n", dfval);
	dev_info_register("CTP",dfdev_info);
#endif
//huaqin add by xudongfang for dev_info at 2018/3/15 end
    FTS_FUNC_EXIT();
    return 0;
...
#if FTS_TEST_EN
    df_fts_test_exit(&hwinfo_device);
#endif
//huaqin add by xudongfang for temporary ito test at 2018/2/11 end
