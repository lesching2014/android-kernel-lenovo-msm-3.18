...
//huaqin modify by xudongfang for 2nd  ito test at 2018/3/21 start
    sprintf(filepath, "%s%s", FTS_CSV_FILE_PATH, file_name);
//huaqin modify by xudongfang for 2nd  ito test at 2018/3/21 end
    if (NULL == pfile) {
...
static bool fts_test_start(void)
{
    ...
    if (test_data.func && test_data.func->start_test) {
        ...
        testresult = false;
//huaqin modify by xudongfang for finally ito test at 2018/3/26 start
		ito_TestResult_fts=0;
//huaqin modify by xudongfang for finally ito test at 2018/3/26 end
    }
    FTS_TEST_FUNC_EXIT();
    return testresult;
}
...
//huaqin add by xudongfang for temporary ito test at 2018/2/11 start
//*****************************************ito test start  ********************************************
static ssize_t df_fts_test_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	count= sprintf(buf,"%d\n",ito_TestResult_fts);
	printk("DF_ito_fts_test_show();\n");
    return count;
}

static ssize_t df_fts_test_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    return count;
}
...
*    example:echo "***.ini" > fts_test
*/
static DEVICE_ATTR(factory_check, 0644,df_fts_test_show,df_fts_test_store);

/* add your attr in here*/
static struct attribute *df_fts_test_attributes[] = {
    &dev_attr_factory_check.attr,
    NULL
};

static struct attribute_group df_fts_test_attribute_group = {
    .attrs = df_fts_test_attributes
};

int df_fts_test_init(struct platform_device * tpinfo_device)
{
    int ret = 0;

    ret = sysfs_create_group(&tpinfo_device->dev.kobj, &df_fts_test_attribute_group);
    if (0 != ret) {
        FTS_TEST_ERROR( "DF %s() - ERROR: sysfs_create_group() failed.",  __func__);
        sysfs_remove_group(&tpinfo_device->dev.kobj, &df_fts_test_attribute_group);
    } else {
        FTS_TEST_DBG("DF %s() - sysfs_create_group() succeeded.", __func__);
    }
...
int df_fts_test_exit(struct platform_device * tpinfo_device)
{
