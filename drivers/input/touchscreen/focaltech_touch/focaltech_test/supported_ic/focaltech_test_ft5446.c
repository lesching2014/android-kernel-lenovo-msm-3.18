...
unsigned char FT5X46_TestItem_PanelDifferTest(bool *bTestResult)
{
    ...
    //Determine whether for v3 TP first, and then read the value of the 0x54
    //and check the mapping type is right or not,if not, write the register
    //rawdata test mapping\A1\EA?before mapping\A1\EAo0x54=1;after mapping\A1\EAo0x54=0;
    if (m_bV3TP) {
        ReCode = read_reg( REG_MAPPING_SWITCH, &strSwitch );
        if (ReCode != ERROR_CODE_OK) {
            FTS_TEST_SAVE_INFO("\n\n Read REG_MAPPING_SWITCH error. Error Code: %d",  ReCode);
    ...
    //change register value before,need to lose 3 frame data\A1\EA?4th frame data is valid
...
unsigned char FT5X46_TestItem_WeakShortTest( bool *bTestResult )
{
    ...
    for (i = 0; i < iChannelNum; i++) {
        ...
        if ((iMin_CG > fGShortResistance[i]) || (iDsen - iDoffset < 0)) { //<=0 ShortResistance = 0
            iCount++;
            if (i + 1 <= iMaxTx)
                FTS_TEST_SAVE_INFO("\nTx%02d: %02d (k\A6\B8),	", i + 1, fGShortResistance[i]);
            else
                FTS_TEST_SAVE_INFO("\nRx%02d: %02d (k\A6\B8),	", i + 1 - iMaxTx, fGShortResistance[i]);
            if (iCount % 10 == 0)
                FTS_TEST_SAVE_INFO("\n\n");
        }
    ...
    for ( i = 0; i < iChannelNum; i++) {
        ...
        if ( fMShortResistance[i] <= 0  || fMShortResistance[i] < iMin_CC ) {
            iCount++;
            if (i + 1 <= iMaxTx)
                FTS_TEST_SAVE_INFO("\nTx%02d: %02d(k\A6\B8),	", i + 1, fMShortResistance[i]);
            else
                FTS_TEST_SAVE_INFO("\nRx%02d: %02d(k\A6\B8),	", i + 1 - iMaxTx, fMShortResistance[i]);
