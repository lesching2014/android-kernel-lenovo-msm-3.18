...
//Hua Qin add by tangqingyong at 2018/02/23 For Platform & lenovo ROM start
#define SUPPORT_THERMAL_MITIGATION_CTRL_CURRENT
//Hua Qin add by tangqingyong at 2018/02/23 For Platform & lenovo ROM end

//Hua Qin add by tangqingyong at 2018/02/11 For Platform & lenovo ROM start
#define SUPPORT_PM_AWAKE_USB_INSERTION
//Hua Qin add by tangqingyong at 2018/02/11 For Platform & lenovo ROM end

#define SUPPORT_FG_RESET_STATUS_CHECK
#ifdef SUPPORT_FG_RESET_STATUS_CHECK
...
struct smbchg_chip {
	...
	int				rslow_uohm;
	int				vled_max_uv;
//LENOVO by tianye at 2018/02/13 For Platform & lenovo ROM start
...
//Hua Qin add by tangqingyong at 2018/04/13 For Platform & lenovo ROM start
	int				cool_cc_comp_deep;
//Hua Qin add by tangqingyong at 2018/04/13 For Platform & lenovo ROM end
	struct work_struct		custom_jeita_work;
#endif
#ifdef SUPPORT_SMBCHG_MONITOR
	struct delayed_work		custom_monitor_work;
#endif
//LENOVO add by tianye at 2018/02/13 For Platform & lenovo ROM end
//Hua Qin add by tangqingyong at 2018/02/27 For Platform & lenovo ROM start
...
enum wake_reason {
	...
	//Hua Qin add by tangqingyong at 2018/02/11 For Platform & lenovo ROM start
#ifdef SUPPORT_PM_AWAKE_USB_INSERTION
	PM_USB_INSERTION = BIT(5),
#endif
//Hua Qin add by tangqingyong at 2018/02/11 For Platform & lenovo ROM end
};
...
//Hua Qin add by tangqingyong at 2018/02/23 For Platform & lenovo ROM start
#ifdef SUPPORT_THERMAL_MITIGATION_CTRL_CURRENT
#define THERMAL_CHG_FCC_VOTER		"THERMAL_CHG_FCC_VOTER"
#endif
//Hua Qin add by tangqingyong at 2018/02/23 For Platform & lenovo ROM end
/* ICL VOTERS */
...
//Hua Qin add by tangqingyong at 2018/02/23 For Platform & lenovo ROM start
	/*enable and disable battery FCC limit current*/
#ifdef SUPPORT_THERMAL_MITIGATION_CTRL_CURRENT
#define BATTCHG_THERMAL_BATTERY_EN_VOTER "BATTCHG_THERMAL_BATTERY_EN_VOTER"
#endif
//Hua Qin add by tangqingyong at 2018/02/23 For Platform & lenovo ROM end
...
#define USB_DP          0x43
#define USB_DM          0x44
int smbchg_get_usbin_uv(struct smbchg_chip *chip)
{
        int rc;
        ...
        chip->adc_usbin_uv = (int)adc_result.physical;
        ...
        chip->adc_dp_uv = (int)adc_result.physical;
        ...
        chip->adc_dm_uv = (int)adc_result.physical;
...
//Hua Qin add by tangqingyong at 2018/02/23 For Platform & lenovo ROM start
#ifdef SUPPORT_THERMAL_MITIGATION_CTRL_CURRENT
		rc = vote(chip->battchg_suspend_votable, BATTCHG_THERMAL_BATTERY_EN_VOTER, true, 0);

		if (rc < 0) {
                        dev_err(chip->dev,
                                "Couldn't set BATT suspend rc %d\n", rc);
                        goto out;
                }
...
//Hua Qin add by tangqingyong at 2018/02/23 For Platform & lenovo ROM start
#ifdef SUPPORT_THERMAL_MITIGATION_CTRL_CURRENT
		rc = vote(chip->battchg_suspend_votable, BATTCHG_THERMAL_BATTERY_EN_VOTER, false, 0);
		if (rc < 0)
                        pr_err("Couldn't disable BATT thermal FCC vote rc=%d\n",
                                rc);
#else
//Hua Qin add by tangqingyong at 2018/02/23 For Platform & lenovo ROM end
...
//Hua Qin add by tangqingyong at 2018/02/23 For Platform & lenovo ROM start
		...
		rc = vote(chip->fcc_votable, THERMAL_CHG_FCC_VOTER, true,
					thermal_fcc_ma);
		pr_info("thermal_fcc_ma = %d\n", thermal_fcc_ma);
		if (rc < 0)
                        pr_err("Couldn't vote for BATT thermal FCC rc=%d\n", rc);
...
static void smbchg_ns_charger_monitor_work(struct work_struct *work)
{
        ...
        effective_client_str = get_effective_client_locked(chip->usb_icl_votable);
        pr_smb(PR_STATUS, "voter: fcc = %d, icl = %d, icl_vote_str = %s, usb_suspend = %d, battchg_suspend = %d\n",
                                fcc_ma, usb_icl_ma, effective_client_str, usb_suspend, battchg_suspend);
        if (!chip->boot_done) {
						...
						enum power_supply_type type)
{
	 ...
	 */
/* Huaqin add for charger bring up by tangqingyong at 2018/2/7 start*/
	if (type == POWER_SUPPLY_TYPE_USB)
/* Huaqin add for charger bring up by tangqingyong at 2018/2/7 end*/
...
#ifdef SUPPORT_PM_AWAKE_USB_INSERTION
	if (!chip->usb_present) {
		pr_smb(PR_MISC, "PM Relax: PM_USB_INSERTION\n");
		smbchg_relax(chip, PM_USB_INSERTION);
	}
#endif
//Hua Qin add by tangqingyong at 2018/02/11 For Platform & lenovo ROM end
...
#ifdef SUPPORT_PM_AWAKE_USB_INSERTION
	if (chip->usb_present) {
		pr_smb(PR_MISC, "PM Awake: PM_USB_INSERTION\n");
		smbchg_stay_awake(chip, PM_USB_INSERTION);
	}
#endif
//Hua Qin add by tangqingyong at 2018/02/11 For Platform & lenovo ROM end
...
#ifdef SUPPORT_FG_RESET_STATUS_CHECK
	POWER_SUPPLY_PROP_FG_RESET_STATUS,
#endif
};
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_MAX_PULSE_ALLOWED:
		...
		break;
#ifdef SUPPORT_FG_RESET_STATUS_CHECK
	case POWER_SUPPLY_PROP_FG_RESET_STATUS:
		val->intval = fg_reset_status;
		if (fg_reset_status)
			pr_info("FG reset status is True.\n");
		break;
#endif
	default:
		return -EINVAL;
...
//LENOVO add by tianye at 2018/02/13 For Platform & lenovo ROM start
#ifdef SUPPORT_JEITA_TEMP_PATCH
	schedule_work(&chip->custom_jeita_work);
#endif
//LENOVO add by tianye at 2018/02/13 For Platform & lenovo ROM end
	smbchg_parallel_usb_check_ok(chip);
	if (chip->psy_registered)
...
//LENOVO add by tianye at 2018/02/13 For Platform & lenovo ROM start
#ifdef SUPPORT_JEITA_TEMP_PATCH
static int smbchg_float_voltage_comp_get(struct smbchg_chip *chip)
{
	int rc;
	u8 reg;
 ...
 *******************************************/
 //Hua Qin add by tangqingyong at 2018/04/13 For Platform & lenovo ROM end
static void custom_smbchg_jeita_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				custom_jeita_work);
	...
	if (chip->batt_warm) {
		...
		if (rc < 0)
			...
			pr_smb(PR_STATUS, "JEITA Voter warm_cc_comp %dmA\n", chip->warm_cc_comp);
	} else if (chip->batt_cool) {
		rc = vote(chip->fcc_votable, JEITA_CUSTOM_FCC_VOTER, true,
				chip->cool_cc_comp);
		if (rc < 0)
			...
			pr_smb(PR_STATUS, "JEITA Voter cool_cc_comp %dmA\n", chip->cool_cc_comp);
	} else {
		...
		if (rc < 0)
			...
			pr_smb(PR_STATUS, "JEITA Voter is disabled.\n");
	}
}
#endif

#ifdef SUPPORT_SMBCHG_MONITOR
#define CUSTOM_SMBCHG_MONITOR_MS			10000
static void custom_smbchg_monitor_work(struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				custom_monitor_work.work);
//Hua Qin add by tangqingyong at 2018/04/13 For Platform & lenovo ROM start
#ifdef SUPPORT_JEITA_TEMP_PATCH
#define TEMP_WARM_THRESHOLD				450
#define TEMP_COOL_THRESHOLD				200
#define TEMP_COOL_THRESHOLD_DEEP			100
#define BATT_FULL_CAPACITY				100
#define TEMP_WARM_RESUME_CHARGE_CHECK			435
//Hua Qin add by tangqingyong at 2018/04/13 For Platform & lenovo ROM end
	static int cool_temp_level_deep;
	int temp = get_prop_batt_temp(chip), rc;

	if ((temp > TEMP_WARM_THRESHOLD) && (smbchg_float_voltage_comp_get(chip) != chip->warm_fv_comp)) {
		rc = smbchg_float_voltage_comp_set(chip, chip->warm_fv_comp);
		if (rc < 0) {
			dev_err(chip->dev, "Couldn't set float voltage comp 0x10 rc = %d\n",
				rc);
		}
		pr_smb(PR_STATUS, "JEITA vfloat_comp %d %dC\n", chip->warm_fv_comp, temp);
	} else if ((temp < TEMP_COOL_THRESHOLD) && (smbchg_float_voltage_comp_get(chip) != chip->normal_fv_comp)) {
		rc = smbchg_float_voltage_comp_set(chip, chip->normal_fv_comp);
		if (rc < 0) {
			dev_err(chip->dev, "Couldn't set float voltage comp 0x0 rc = %d\n",
				rc);
		}
		pr_smb(PR_STATUS, "JEITA vfloat_comp %d %dC\n", chip->normal_fv_comp, temp);
	}
...
//Hua Qin add by tangqingyong at 2018/04/13 For Platform & lenovo ROM start
	if (chip->batt_cool) {
		if((0 <= temp) && (TEMP_COOL_THRESHOLD_DEEP > temp) && (0 == cool_temp_level_deep)){
			cool_temp_level_deep= 1;
			rc = vote(chip->fcc_votable, JEITA_CUSTOM_FCC_VOTER, true,
					chip->cool_cc_comp_deep);
			if (rc < 0)
				dev_err(chip->dev, "Couldn't vote fastchg ma to cool level 1 rc = %d\n", rc);
			else
				pr_smb(PR_STATUS, "JEITA Voter cool level deep FCC = %dmA\n",chip->cool_cc_comp_deep);
		}else if((TEMP_COOL_THRESHOLD_DEEP <= temp) && (TEMP_COOL_THRESHOLD > temp ) && (1 == cool_temp_level_deep)){
			cool_temp_level_deep= 0;
			rc = vote(chip->fcc_votable, JEITA_CUSTOM_FCC_VOTER, true,
					chip->cool_cc_comp);
			if (rc < 0)
				dev_err(chip->dev, "Couldn't vote fastchg ma to cool level 1 rc = %d\n", rc);
			else
				pr_smb(PR_STATUS, "JEITA Voter cool level 0 FCC = %dmA\n",chip->cool_cc_comp);
		}
	}
//Hua Qin add by tangqingyong at 2018/04/13 For Platform & lenovo ROM end
#endif

	schedule_delayed_work(&chip->custom_monitor_work,
					msecs_to_jiffies(CUSTOM_SMBCHG_MONITOR_MS));
}
#endif
//LENOVO add by tianye at 2018/02/13 For Platform & lenovo ROM end
...
//Hua Qin add by tangqingyong at 2018/02/13 For Platform & lenovo ROM start
#ifdef SUPPORT_JEITA_TEMP_PATCH
#define CHG_SOFT_LIMIT_FV_CC_COMP_MASK		SMB_MASK(3, 0)
#define CHG_SOFT_LIMIT_CC_COMP_MASK		SMB_MASK(1, 0)
#endif
//Hua Qin add by tangqingyong at 2018/02/13 For Platform & lenovo ROM end
...
//LENOVO add by tianye at 2018/02/13 For Platform & lenovo ROM start
	...
	OF_PROP_READ(chip, chip->normal_fv_comp, "normal-fv-comp", rc, 1);
//Hua Qin add by tangqingyong at 2018/04/13 For Platform & lenovo ROM start
	OF_PROP_READ(chip, chip->cool_cc_comp_deep, "cool-cc-comp-deep", rc, 1);
//Hua Qin add by tangqingyong at 2018/04/13 For Platform & lenovo ROM end
#endif
//LENOVO add by tianye at 2018/02/13 For Platform & lenovo ROM end
...
//LENOVO add by tianye at 2018/02/13 For Platform & lenovo ROM start
#ifdef SUPPORT_JEITA_TEMP_PATCH
	INIT_WORK(&chip->custom_jeita_work, custom_smbchg_jeita_work);
	INIT_DELAYED_WORK(&chip->custom_monitor_work, custom_smbchg_monitor_work);
#endif
//LENOVO add by tianye at 2018/02/13 For Platform & lenovo ROM end
//Hua Qin add by tangqingyong at 2018/02/27 For Platform & lenovo ROM start
#ifdef SUPPORT_NON_STANDARD_CHARGER
