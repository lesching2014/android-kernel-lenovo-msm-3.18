...
static int mdss_fb_blank_unblank(struct msm_fb_data_type *mfd)
{
	...
	if (mdss_panel_is_power_off(cur_power_state)) {
		...
		if (!mfd->allow_bl_update) {
			mfd->allow_bl_update = true;
//Hua Qin add by duchangguo at 2018/03/02 adding delsy before backlight on For Platform & lenovo ROM start
			msleep(20);
//Hua Qin add by duchangguo at 2018/03/02 adding delsy before backlight on For Platform & lenovo ROM end
			/*
