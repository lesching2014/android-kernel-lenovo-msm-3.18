	...
	struct msm_sensor_info_t *probed_info, char *entity_name)
{
	...
	uint32_t                             is_yuv;
/*huaqin yinyapeng add for camera dev node info -->*/
#ifdef CONFIG_HQ_DEV_INFO
    char camera_pos[MAX_CAMERAS][32] = {
                           "camera_back ",
                           "camera_front",
                           "camera_aux  ",
                           };
...
/*huaqin yinyapeng add for camera dev node info -->*/
#ifdef CONFIG_HQ_DEV_INFO
    switch (slave_info->camera_id) {
       case CAMERA_0:
             rc =  dev_info_register(camera_pos[CAMERA_0],slave_info->sensor_name);
             break;

       case CAMERA_1:
             rc =  dev_info_register(camera_pos[CAMERA_1],slave_info->sensor_name);
            break;

       case CAMERA_2:
             rc =  dev_info_register(camera_pos[CAMERA_2],slave_info->sensor_name);
            break;
       default:
            break;
    }
