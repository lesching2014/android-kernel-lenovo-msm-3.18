...
#define ENABLE_SENSOR_ID_PIN_CHECK

#ifdef ENABLE_SENSOR_ID_PIN_CHECK
  #define SENSOR_SUB_MODULE_1     "sp2509_hlt_austin"
  #define SENSOR_SUB_MODULE_2     "sp2509_sunny_austin"
  #define SENSOR_SUB_GPIO_ID       128

  static sensor_id_pin_info_t sensor_same_ic_info[] = {
            {SENSOR_SUB_GPIO_ID,false, SENSOR_SUB_MODULE_1},
            {SENSOR_SUB_GPIO_ID,true,  SENSOR_SUB_MODULE_2},
  };
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if(gpio_is_valid(SENSOR_SUB_GPIO_ID)){
		...
		for (i=0; i < 2; i++) {
			if(!strcmp(sensor_name, sensor_same_ic_info[i].sensor_name)
				&&  sensor_same_ic_info[i].status != gpio_get_value(sensor_same_ic_info[i].gpio) ){
				pr_err("%s:%d %s check pin %d failed: \n",__func__, __LINE__, sensor_name,SENSOR_SUB_GPIO_ID);
				return -EINVAL;
			}
		}
	}
